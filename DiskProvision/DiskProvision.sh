...
check_mkfs_fat() {
    ...
    if command -v mkfs.fat >/dev/null 2>&1; then
        echo "mkfs.fat is installed."
        return 0
    else
        echo "mkfs.fat is not installed."
        return 1
    fi
}
...
check_qemu_nbd() {
    ...
    if command -v qemu-nbd >/dev/null 2>&1; then
        ...
        echo "qemu-nbd is not installed."
        return 1
    fi
}
...
if [[ "$(uname)" == "Linux" ]]; then
    ...
    # Check for qemu-img comand, install via Homebrew if not found
    if ! check_qemu_img; then
        echo "Attempting to install qemu-img..."
        install_packages_brew qemu
    fi
...
echo "Running DiskProvision using $(python_executable)"
